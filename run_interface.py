...
from ppo_interface import PPO
import torch
from interface_reward_predictor import RewardModel
...
from interface import RateInterface
...
for seed in seeds:
    for num_ratings in num_ratings_total:
        ...
        interface = RateInterface(feed_type=1, num_ratings=num_ratings, reward_model=reward_model, max_num_feedback=max_num_feedback)
        ...
        model = PPO(reward_model, "MlpPolicy", env, learning_rate=lr, n_steps=n_steps ,seed=seed, batch_size=policy_batch, ent_coef=ent_coef, policy_kwargs=policy_kwargs, use_sde=use_sde, sde_sample_freq=sde_freq, gae_lambda=gae_lambda, clip_range=clip_range, update_every=update_every, num_ratings=num_ratings, segment_len=seg_size, verbose=1, tensorboard_log="./tests/" + env_name + "_" + task_name + "_" + str(num_ratings) +"_seed_" +str(seed) + "/")
