    ...
    def train_soft_reward(self):
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                for label in labels:
                    num_ratings[int(label[0])] += 1
                labels = torch.from_numpy(labels.flatten()).long().to(device).to(device)
                target_onehot = F.one_hot(labels, num_classes=self.num_ratings).to(device)
                if member == 0:
                    total += labels.size(0)
                r_hat1 = self.r_hat_member(sa_t_1, member=member)
                r_hat1 = r_hat1.sum(axis=1)
                r_hat = r_hat1.to(device)
                pred = ((r_hat) - torch.min(r_hat)) / (torch.max(r_hat) - torch.min(r_hat)).to(device)
                sorted_indices = pred[:, 0].sort()[1]
                np_pred = pred[sorted_indices].tolist()
                bounds = [torch.as_tensor([0]).to(device)] * (self.num_ratings + 1)
                for i in range(self.num_ratings):
                ...
                for i in range(len(bounds) - 1):
                    Q[i] = -(self.k) * (pred - bounds[i]) * (pred - bounds[i+1])
                our_Q = torch.cat(Q, axis=-1)
                curr_loss = self.softXEnt_loss(our_Q, target_onehot).to(device)
                loss += curr_loss
                ensemble_losses[member].append(curr_loss.item())
                _, predicted = torch.max(our_Q, 1)
                correct = (predicted == labels).sum().item()
