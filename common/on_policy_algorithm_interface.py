...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            if self.reward_model.feedback_flag:
                rgb_array = self.env.render()

                rgb_array = np.expand_dims(rgb_array, axis=1) # num_env x 1000 x 84 x 84 x 3
            ...
            if self.traj_obsact is None:
                ...
                if  self.reward_model.feedback_flag:
                    self.traj_render = np.concatenate((self.traj_render, rgb_array), axis=1)
            ...
            if num_dones > 0:
                # Add trajectories to the reward model
                # Trajectories here are full rollouts of length 1000
                
                self.reward_model.add_data_batch(self.traj_obsact, self.traj_reward, self.traj_render)
                ...
                # Reset trajectories
                self.traj_obsact, self.traj_reward, self.traj_render = None, None, None
                
                
                # Train Reward
                if self.num_updates >= self.update_every and self.reward_model.feedback_flag:
